...
class Settings:
    ...
    def validate(self) -> None:
        """Fail fast on config that would otherwise break deep into a run."""
        errors: list[str] = []
        ...
        if self.repeated_min_count <= 0:
            errors.append("REPEATED_MIN_COUNT must be > 0")
        if self.layering_min_levels <= 0:
            errors.append("LAYERING_MIN_LEVELS must be > 0")
        if self.flicker_window_sec <= 0:
            errors.append("FLICKER_WINDOW_SEC must be > 0")
        if self.flicker_min_events <= 0:
            errors.append("FLICKER_MIN_EVENTS must be > 0")
        if not 0 < self.imbalance_min_ratio <= 1:
            errors.append("IMBALANCE_MIN_RATIO must be in (0, 1]")
        if self.imbalance_min_levels <= 0:
            errors.append("IMBALANCE_MIN_LEVELS must be > 0")
        ...
        if self.spoof_min_price_move <= 0:
            # spoof_pull.py divides by this; a value of 0 is a guaranteed
            # ZeroDivisionError on the first detected price move.
        ...
        if self.risk_aggregation:
            ...
            if not 0 <= self.risk_clear_threshold < self.risk_alert_threshold:
                # Otherwise the elevated state can never clear (or flaps
                # every tick if the two thresholds are reversed).
                errors.append(
        ...
        if self.dashboard_port <= 0 or self.dashboard_port > 65535:
            errors.append("DASHBOARD_PORT must be in 1..65535")
