...
async def create_feed(settings) -> OrderbookFeed:
    """Return a connected feed based on the configured network.

    NETWORK=hyperliquid → HyperliquidOrderbookFeed (public REST API, no SDK)
    NETWORK=drift (default) → DriftOrderbookFeed, falling back to synthetic
    """
    ...
    if network == "hyperliquid":
        ...
        try:
            ...
            logger.warning("Hyperliquid feed unavailable (%s); falling back to synthetic.", exc)
            synthetic = SyntheticOrderbookFeed(settings)
            await synthetic.connect()
            return synthetic

    # Default: Drift DLOB feed, with synthetic fallback.
    try:
