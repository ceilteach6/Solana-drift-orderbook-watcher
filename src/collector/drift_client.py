...
exposing synchronous L2 orderbook snapshots.

Imported lazily by :class:`DriftOrderbookFeed`; if ``driftpy`` (or its
dependencies) is not installed, the ImportError surfaces cleanly and the feed
factory falls back to the synthetic feed.

NOTE: ``driftpy``'s public API shifts across releases.  The setup below follows
the layering documented in the project README (OrderSubscriber -> DLOBSubscriber).
If your installed ``driftpy`` version differs, adjust the imports/constructors
here — the rest of the watcher only depends on ``get_l2(market, depth)``
returning an object with ``bids`` / ``asks``.
"""
...
class DriftStack:
    ...
    def __init__(self, drift_client, dlob_subscriber, connection,
                 user_map=None, slot_subscriber=None) -> None:
    ...
    async def build(cls, settings) -> "DriftStack":
        ...
        # Local imports: a missing driftpy raises ImportError at call time,
        # which the feed factory catches to trigger the synthetic fallback.
    ...
    def get_l2(self, market: str, depth: int = 20):
        """Return the current L2 orderbook for ``market``."""
        return self._dlob.get_l2_orderbook_sync(market, depth=depth)

    async def close(self) -> None:
        # Unsubscribe in reverse dependency order: DLOB → slot → user_map → client.
