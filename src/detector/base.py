...
Detector base class, the :class:`Detection` result type, and the size
clustering helper shared by the repeated-size and layering detectors.

To add a new detector: subclass :class:`BaseDetector`, implement ``analyze``,
and register it in ``src/detector/__init__.py`` DEFAULT_DETECTORS.
"""
...
class BaseDetector:
    ...
    def analyze(self, snapshot, history) -> list[Detection]:
        ...
        Args:
            snapshot: the current :class:`~src.collector.orderbook_feed.OrderbookSnapshot`.
...
def cluster_sizes(sizes, tolerance: float) -> list[_Cluster]:
    """Greedily group near-equal sizes using relative tolerance.
