...
### Replay / backtesting ✅
- **Replay** (`src/replay.py`) — a `PERSIST_SNAPSHOTS=true` mellett rögzített
  L2 könyveket visszajátssza a élő detektor-stacken + risk-aggregátoron,
  read-only (nem ír a `detections`/`risk` táblákba, tehát egy élő futás
  history-ját sosem rontja el). Küszöbhangoláshoz: futtasd újra más
  `.env`-thresholdokkal, és nézd meg, melyik detektor hányszor sült el.
  `python main.py --replay [--replay-market SOL-PERP]`. *(kész)*
