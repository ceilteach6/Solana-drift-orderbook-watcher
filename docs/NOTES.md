...
- **Replay** (`src/replay.py`) — a perzisztált L2 snapshotokat (`PERSIST_SNAPSHOTS=true`)
  újrajátssza egy friss detektor + risk-aggregátor + alert példányon keresztül,
  így küszöbérték-hangolás végezhető a rögzített történelmen anélkül, hogy a
  live feedhez vagy a mentett detekciókhoz hozzáérnénk. `python main.py --replay
  SOL-PERP [--speed N]` (`--speed` = realtime szorzó; 0 = lehető leggyorsabban). *(kész)*
