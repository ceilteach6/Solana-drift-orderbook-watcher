...
| `src/replay.py` — detektor replay/backtest perzisztált könyveken (`--replay`) | ✅ kész (új) |
...
### Replay / backtesting ✅
- **`src/replay.py`** — a valós detektor-stacket (és a risk-aggregátort) újra
  lefuttatja a `PERSIST_SNAPSHOTS=true` mellett elmentett L2 könyveken, élő
  kapcsolat nélkül. `python main.py --replay [--replay-market SOL-PERP]
  [--replay-limit N]`. Erre épül a küszöbhangolás. *(kész)*
